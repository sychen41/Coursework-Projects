...
(function ($, window, document) {
	...
	******************************************************************************/
	var defaults = {
		...
		onClickEvent: null							// On-Click event that user can specify
	};
	...
	function Zoomer($elem, params) {
		...
		**********************************************************/
		...
		this.onClickEvent = op.onClickEvent;
	...
	Zoomer.prototype = {
		...
		***********************************************************************************************************************/
		...
		/*Set a new image*/
		setToImage: function(path){
			console.log("Set img src to " + path);
			this.$image.attr('src', path);
			this.image_url = path;
		},

		/*Check the click point we have just hit based on another point (center of target) and see if it is within 'tolerance' proximity (true/false) */
		checkClickPoint: function(clickX, clickY, pointX, pointY, tolerance){
			pointX = pointX * this.rA;
			pointY = pointY * this.rA;
			console.log("Checking click point: (" + clickX + "," + clickY + ") vs (" + pointX + "," + pointY + ")  tolerance:" + tolerance);

			var tolXMin = pointX - tolerance;
			var tolXMax = pointX + tolerance;
			var tolYMin = pointY - tolerance;
			var tolYMax = pointY + tolerance;

			console.log("Tolerance Adjustment:" + tolXMin + "," + tolXMax + "," + tolYMin + "," + tolYMax);

			// now, see if the users click is within these tolerance pixels
			if(clickX > tolXMin && clickX < tolXMax && clickY > tolYMin && clickY < tolYMax){
				console.log("YES");
				return true;
			}
			...
			if (self.cFlag._nd) {
				...
				if (e.type == 'mousedown'){			
					...
					if (te.targetTouches.length > 1) {
						...
						self.stX = te.touches[0].pageX;
						self.stY = te.touches[0].pageY;
						self.endX = te.touches[0].pageX;
						self.endY = te.touches[0].pageY;
			...
			if (e.type == 'mousemove') {
				...
				if (self._onHitArea) {
					...
					if (self._pinching || touches.length >1) {
						...
						self.endX = touches[0].pageX;
						self.endY = touches[0].pageY;
				...
				if (self.mouse_drag || self.touch_drag) {
					...
					if (e.type == 'mouseup') {
						...
						if (self.stX ==  e.pageX && self.stY == e.pageY) {
							...
							if(self.onClickEvent != null){
								self.onClickEvent(self.offX, self.offY);
							}
						}						
						self._recent = 'drag';
						self._dragging = false;
					...
					//Touch
						...
						if (self._pinching) {
							...
							if (self.stX ==  self.endX && self.stY == self.endY) {
								if(self.onClickEvent != null){
									self.onClickEvent(self.offX, self.offY);
								}
							}
