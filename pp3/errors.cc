...
void ReportError::DeclConflict(Decl *decl, Decl *prevDecl) {
    stringstream s;
    s << "Declaration of '" << decl->id->name << "' here conflicts with declaration on line " 
...
void ReportError::InterfaceNotImplemented(Decl *cd, Decl *decl) {
    stringstream s;
    s << "Class '" << cd->id->name << "' does not implement entire interface '" << decl->id->name << "'";
    OutputError(decl->GetLocation(), s.str());
}
...
void ReportError::IdentifierNotDeclared(Identifier *ident, reasonT whyNeeded) {
    ...
    s << "No declaration found for "<< names[whyNeeded] << " '" << ident->name << "'";
