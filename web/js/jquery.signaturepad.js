...
function SignaturePad (selector, options) {
  ...
  function setPenWidth(newPenWidth){
    settings.penWidth = newPenWidth;
  ...
  var savedPaths = null;
  var savedPathsIdx = 0;
  var savedPathsSaveOutput = false;
  var savedPathsContext = null;
  function slowDrawSignature(paths, context, saveOutput) {
    savedPaths = paths;
    savedPathsIdx = -1;
    savedPathsSaveOutput = saveOutput;
    savedPathsContext = context;
    continueSlowDrawSignature();
  }

  function continueSlowDrawSignature(){
    savedPathsIdx++;
    if(savedPathsIdx < savedPaths.length){
      // draw!
      if (typeof savedPaths[savedPathsIdx] === 'object') {
        savedPathsContext.beginPath()
        savedPathsContext.moveTo(savedPaths[savedPathsIdx].mx, savedPaths[savedPathsIdx].my)
        savedPathsContext.lineTo(savedPaths[savedPathsIdx].lx, savedPaths[savedPathsIdx].ly)
        savedPathsContext.lineCap = settings.penCap
        savedPathsContext.stroke()
        savedPathsContext.closePath()

        if (savedPathsSaveOutput) {
          output.push({
            'lx' : savedPaths[savedPathsIdx].lx
            , 'ly' : savedPaths[savedPathsIdx].ly
            , 'mx' : savedPaths[savedPathsIdx].mx
            , 'my' : savedPaths[savedPathsIdx].my
          })
        }
      }
      
      var _this = self;
      // darwing done, set a timeout to render next path
      setTimeout(function() { continueSlowDrawSignature(); }, settings.slowDrawTime);
    }
    else{
      // else we end gracefully
      savedPaths = null;
      savedPathsIdx = 0;
      savedPathsSaveOutput = false;
      savedPathsContext = null;
    }
  }
  ...
  $.extend(self, {
    ...
    /**
     * Regenerates a signature on the canvas using an array of objects
     * Follows same format as object property SLOWLY
     * @see var object
     *
     * @param {Array} paths An array of the lines and points
     */
    , regenerateSlow : function (paths) {
      self.clearCanvas()
      $(settings.typed, context).hide()
      ...
      slowDrawSignature(paths, canvasContext, true)

      if (settings.output && $(settings.output, context).length > 0)
        $(settings.output, context).val(JSON.stringify(output))
    }

    , setPenWidth : function (newPenWidth) {
      settings.penWidth = newPenWidth;
    }
    ...
    , getCanvas : function () { return element; }
...
$.fn.signaturePad.defaults = {
  ...
  , lineMargin : 0 // Margin on right and left of signature line
  , lineTop : 0 // Distance to draw the line from the top
  ...
  , slowDrawTime : 50  // Number of MS to wait for each path to be drawn in the 'slowRegenerate' method
}
}(jQuery))
